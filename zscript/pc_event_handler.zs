...
/*
  This EventHandler is a modified version of the original Precise Crosshair one.
*/
...
// private: ////////////////////////////////////////////////////////////////////
  ...
  {   
    int pressedKey = e.Args[0];
    State runningState = pawn.FindState("Running");
    State seeState = pawn.FindState("See");
    State deathState = pawn.FindState("Death");
    int clRun = CVar.FindCVar("cl_run").GetInt();
    if (e.Name == "KEYPRESS") {
        console.printf("%d", pressedKey);
        if (pressedKey == 42 || clRun == 1) {
          isRunning = true;
      ...
      if (pressedKey == 42 || clRun == 1) {
          isRunning = false;
          pawn.SetState(seeState, true);
      }
      if (pressedKey == 258) {
        isFrontView = false;
      }
    }
    // This line is responsible for keeping the player on the death state,
    // even if the user presses a movement key for example.
    if (player.health == 0) {
      pawn.SetState(deathState, true);
    }
    if (isFrontView) {
      playerAngle = 0;
    } else {
